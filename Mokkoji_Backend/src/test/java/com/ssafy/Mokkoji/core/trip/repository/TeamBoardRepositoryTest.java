...
class TeamBoardRepositoryTest {
	...
	void search() {
		...
		List<TeamBoardSpecification> result = teamBoardRepository.searchAllTeamBoard(boardSearch,
			tripTeam.getTripTeamId());
	...
	void find() {
		...
		// then
		assertThat(result).extracting(TeamBoardSpecification::getContent)
			.isEqualTo("hello1");
		assertThat(result).extracting(TeamBoardSpecification::getTitle)
			.isEqualTo("hello1");
		assertThat(result).extracting(TeamBoardSpecification::getNickname)
			.isEqualTo(user.getNickname().getValue());
	}
